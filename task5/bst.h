...
public:
    ...
    size_t height() const {
        ...
        auto left_height = _height(_root->_left);
        auto right_height = _height(_root->_right);
...
private:
    ...
    size_t _height(node *node) const {
        if (node == nullptr || node == _most_left) {
            return 0;
        }

        auto left_height = _height(node->_left);
        auto right_height = _height(node->_right);
    ...
    bool _is_avl(node * node) const {
        ...
        return abs(left_height - right_height) <= 1 && _is_avl(node->_left) && _is_avl(node->_right);
    }

    void _swap(bst<T, Cmp> &other) {
